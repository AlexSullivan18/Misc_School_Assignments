...
public class TriviaGame
{
	 ...
	 * @param qArray
	 */
	public static void initQuestions(ArrayList<Question> qArray) throws IOException
	{
		...
		for (int i = 0; i <NUM_QUESTIONS; i++)
		{
			...
			qArray.add(i, new Question());

			// Get the question text from the file.
			qArray.get(i).setQuestion(inputFile.nextLine());

			// Get the possible answers.
			for (int j = 0; j < 4; j++)
			{
				qArray.get(i).setPossibleAnswer(inputFile.nextLine(), j);
			}

			// Get the correct answer.
			qArray.get(i).setCorrectAnswerNumber(Integer.parseInt(inputFile.nextLine()));
		}
	}
